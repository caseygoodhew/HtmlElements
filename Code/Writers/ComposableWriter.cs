...
namespace Coding.Writers
{
    public abstract class ComposableWriter : VariableTypeWriter
    {
        ...
        protected ComposableWriter(string name, Token declaringToken) 
        {
        ...
        {
            builder.Join(Methods, x => x.Write(builder, context), Token.NewLine);
        }

        protected virtual void WriteFields(TokenBuilder builder, WriterContext context)
        {
            builder.Join(Fields, x => x.Write(builder, context), Token.NewLine);
        }

        protected virtual void WriteProperties(TokenBuilder builder, WriterContext context)
        {
            builder.Join(Properties, x => x.Write(builder, context), Token.NewLine);
        }
